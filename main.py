...
async def delete_event(interaction: discord.Interaction, event_id: str):
    ...
    try:
        if not has_event_access(interaction):
            await interaction.response.send_message(
                "❌ Only staff, administrators, or owners can delete events!", ephemeral=True)
            return
        ...
        try:
            ...
            await interaction.response.send_message(
                "❌ Invalid event ID! Use numbers only.", ephemeral=True)
            return
        ...
        try:
            ...
            await event.delete()
            await interaction.response.send_message(
                f"✅ Successfully deleted event with ID: `{event_id}`", ephemeral=True)
        except discord.NotFound:
            await interaction.response.send_message(
                f"❌ No event found with ID: `{event_id}`", ephemeral=True)
        except discord.Forbidden:
            await interaction.response.send_message(
                "❌ I don't have permission to delete events!", ephemeral=True)
        except Exception as e:
            await interaction.response.send_message(
                f"❌ Error deleting event: {str(e)}", ephemeral=True)
...
async def list_events(interaction: discord.Interaction):
    ...
    try:
        ...
        event_list = [
            f"**{e.name}** (ID: `{e.id}`)\n"
            f"UTC: {e.start_time.strftime('%Y-%m-%d %H:%M')} - {e.end_time.strftime('%H:%M')}"
            for e in events
        ]
