...
async def run_service() -> None:
    ...
    if not path.exists():
        logger.warning("backend file (%s) no found! front service may dont work", path)
        return
    ...
    if process.poll() is not None:
        ...
        logger.warning("failed to start backend file!, return code: %i", process.poll())
    else:
...
bot: commands.Bot = commands.Bot(
    command_prefix=config.get("prefix", "!"), intents=intents
)
...
async def on_app_command_error(
    ...
    if interaction.response.is_done():
        send: Callable = interaction.followup.send
    else:
        await interaction.response.defer(ephemeral=True)
...
ACTIVITY_RULES: list[
    Callable[[datetime.datetime, str, str], tuple[bool, discord.activity.BaseActivity]]
] = [
    (
        lambda _, key, value: (
            key in ["music"],
    ...
    (
        lambda _, __, value: (True, discord.activity.CustomActivity(name=value))
    ),  # default
]
...
async def main() -> None:
    ...
    global cogs
    cogs = cogs_finder.list_cogs()
    logger.info("Cogs loaded: %i successful, %i failed.", successcount, failcount)
