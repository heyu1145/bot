...
class ChannelsCog(commands.Cog):
    ...
    channels_group = app_commands.Group(
        name="channels", description="channels options", guild_only=True
    )
    ...
    async def channel_view(
        ...
        if not interaction.guild:
            raise app_commands.CheckFailure("Thid command can only used in server")

        if not channel.permissions_for(channel.guild.me).view_channel:
            raise app_commands.BotMissingPermissions(["view_channel"])

        basic_embed = discord.Embed(
            title=f"{channel.name}'s info", color=0xFF, timestamp=discord.utils.utcnow()
        )
        ...
        match channel:
            case discord.TextChannel(
                nsfw=nsfw, members=members, slowmode_delay=slowmode_delay
            ):
                multi_set_fields(
                    basic_embed,
                    is_nsfw=nsfw,
            ...
            case discord.VoiceChannel(nsfw=nsfw, members=members, bitrate=bitrate):
                multi_set_fields(
                    basic_embed,
                    is_nsfw=nsfw,
                    members=len(members),
